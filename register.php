...
if(is_post()) {
    ...
    if(empty(trim($_POST["nome"]))) {
        $nome_err = "Por favor coloque o nome de sua OM.";
    } else {
        $nome = trim($_POST['nome']);
    }
    
    if(empty(trim($_POST["sigla"]))) {
        $sigla_err = "Por favor insira a sigla de sua OM.";     
    } else {
        $sigla = trim($_POST["sigla"]);
    }

    if(empty(trim($_POST["login"]))) {
        $login_err = "Por favor insira um login para sua OM.";     
    } else {
        $login = trim($_POST["login"]);
    }

    if(empty(trim($_POST["senha"]))) {
        $senha_err = "Por favor insira uma senha para sua OM.";     
    } else {
