...
if(is_post()) {
    ...
    if(empty(trim($_POST["data"]))) {
        $data_err = "Por favor insira uma data.";
    } else {
        $data = trim($_POST['data']);
    }
    
    if(empty(trim($_POST["consumo"]))) {
        $consumo_err = "Por favor insira o consumo de sua OM.";     
    } else {
        $consumo = trim($_POST["consumo"]);
    }

    if(empty(trim($_POST["demanda-medida"]))) {
        $demanda_medida_err = "Por favor insira um demanda para sua OM.";     
    } else {
        $demanda_medida = trim($_POST["demanda-medida"]);
    }

    if(empty(trim($_POST["energia-reativa"]))) {
        $energia_reativa_err = "Por favor insira uma energia reativa para sua OM.";     
    } else {
        $energia_reativa = trim($_POST["energia-reativa"]);
    }

    if(empty(trim($_POST["energia-ativa"]))) {
        $energia_ativa_err = "Por favor insira uma energia ativa para sua OM.";     
    } else {
